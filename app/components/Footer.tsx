...
const SocialCards = ({ mobile = false }: { mobile?: boolean }) => (
  <div className={`relative z-20 flex items-end ${mobile ? "w-full justify-center pb-3 gap-0 min-[400px]:gap-2 translate-x-3" : "ml-1 pb-38"}`}>
    {socialCards.map(({ label, href, card, offset, offset2, offset3, offset4 }) => {
        ...
        >
          ...
          >
            <Image
              ...
              width={126}
              height={168}
              className={`w-auto ${mobile ? "h-20" : "h-44 z-31"}`}
            />
          </a>
        </div>
      );
...
const Footer = () => {
  return (
    ...
    {/*opportunities */}
    <div className="flex w-full flex-col items-center gap-5 lg:w-auto mt-30">
        <h2 className="w-full text-center font-fraunces text-white-100 text-4xl">More Opportunities</h2>

        <div className="flex flex-col gap-4 font-fraunces text-xl">
            {opportunities.map(({title, href}) => (
                <a key={title} href={href} target="_blank" className="px-15 min-[400px]:px-20 py-2 text-center rounded-2xl border-2 border-white-100 text-white-100 shadow-xl bg-linear-to-b from-blue-900 to-blue-900 transition-colors duration-300 ease-out hover:text-blue-900 hover:bg-linear-to-b hover:from-white-100 hover:to-blue-100">
                    {title}
                </a>
            ))}
        </div>
    ...
    <div className="relative z-10 -mt-[5vw] lg:hidden">
      ...
      />
      <p className="absolute inset-x-0 bottom-2 z-40 text-center font-fraunces text-md text-blue-900">
        {`© ${new Date().getFullYear()} Cutie Hack • Made with 💗 and  by ACM Hacks`}
      </p>
    </div>
    {/* desktop ground */}
    <div className="relative hidden -mt-6 lg:block lg:-mt-15">
      <Image
      ...
      <p className="absolute inset-x-0 bottom-2 z-40 text-center font-fraunces text-md text-white-100 lg:text-blue-900 lg:inset-x-auto lg:right-4 2xl:text-xl">
        {`© ${new Date().getFullYear()} Cutie Hack • Made with 💗 and  by ACM Hacks`}
      </p>
    </div>
    </footer>
